...
SkShaper::RunHandler::Buffer RunHandler::runBuffer(const RunInfo& info) {
    int glyphCount = (int)(SkTFitsIn<int>(info.glyphCount) ? info.glyphCount : INT_MAX);
    int utf8RangeSize = (int)(SkTFitsIn<int>(info.utf8Range.size()) ? info.utf8Range.size() : INT_MAX);
    ...
    fClusterOffset = (int) info.utf8Range.begin();
                                 ...
                                 const SkFont& font)
{
    ...
    font.getWidths(glyphs, (int)glyphCount, advances.get());
    ...
    for (size_t glyphIndex = 0; glyphIndex < glyphCount; ++glyphIndex) {
        ...
        unsigned textEnd = (unsigned int)utf8TextBytes;
        ...
        unsigned clusterGlyphCount = (unsigned) (glyphIndex + 1 - clusterStart);
        ...
        while (ptr < end) {  // for each codepoint in cluster
            ...
            int firstIndex = (int)(ptr - utf8Text);
            float nextX = x + width;
